...
from datasets import Dataset as Dataset_hug
...
class DreamBoothDataset(Dataset):
    def __init__(
        ...
        self.datasets_paths = datasets_paths[0]
        self.train_datasets_paths = os.path.join(self.datasets_paths, "train")
        self.test_datasets_paths = os.path.join(self.datasets_paths, "test")
        with open(os.path.join(self.train_datasets_paths, "data.json"), 'r') as file:

            train_json = json.load(file)
        # with open(os.path.join(self.test_datasets_paths, "data.json"), 'r') as file:
       
        #     test_json = json.load(file)
        self.train_data = Dataset_hug.from_dict(train_json)
    ...
    def __getitem__(self, index):
        ...
        # Load image
        image =Image.open(self.train_data[img_idx]["image"])
        image = self.set_image(image, switch)
        ...
        masks = [
            self.set_image(Image.open(self.train_data[img_idx][key]),switch) for key in self.train_data[img_idx] if "mask_1" in key
        ]
...
def main():
    ...
    #     safety_checker=None,
    #     text_encoder=accelerator.unwrap_model(text_encoder), vae=accelerator.unwrap_model(vae),
